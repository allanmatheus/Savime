...
{
    QueryDataManagerPtr newInstance 
    = QueryDataManagerPtr(new DefaultQueryDataManager(_configurationManager, _systemLogger));
    return newInstance;
}
...
int32_t DefaultQueryDataManager::GetQueryId()
{
    return _queryId;
}
...
std::string DefaultQueryDataManager::GetErrorResponse()
{
    return _error;
}
...
std::string DefaultQueryDataManager::GetQueryText()
{
    return _query;
}
...
std::list<std::string> DefaultQueryDataManager::GetParamsList()
{
    ...
    for(std::map<std::string,int>::iterator iter = _blockFiles.begin(); iter != _blockFiles.end(); ++iter)
    {
...
int32_t DefaultQueryDataManager::GetParamFile(std::string paramName)
{
    ...
    {
        std::string filePath = generateUniqueFileName(dir);
...
QueryPlanPtr DefaultQueryDataManager::GetQueryPlan()
{
    return _queryPlan;
}
