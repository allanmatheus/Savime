...
int main(int argc, char** args){
        ...
        try
        {
            ...
            JobManagerPtr jobManager = builder->BuildJobManager();
            
            if(jobManager->Start() == SAVIME_FAILURE)
            {
                throw std::runtime_error("Error during job manager initialization. Abort!");
            }

            if(connectionManager->Start() == SAVIME_FAILURE)
            {
                throw std::runtime_error("Error during connection manager initialization. Abort!");    
            }
