...
/// Modified by Hermano Lustosa to allow run-time multidimensional support. Possibily slower, but more flexible.
///
...
public:
  ...
  int Search(const ELEMTYPE a_min[], const ELEMTYPE a_max[], t_resultCallback a_resultCallback, void* a_context);
  ...
  public:
    ...
    Iterator(int dims_no)                         {NUMDIMS = dims_no; Init(); }
...
protected:
  ...
  struct Branch
  {
    ...
    Branch(int dim_no): m_rect(dim_no){}
  };
  ...
  struct Node
  {
	...
	{
	    Branch branch(dim_no);
	    m_branch.push_back(branch);		
	}
    }
  ...
  struct PartitionVars
  {
    ...
    PartitionVars(int dim_no) : m_coverSplit(dim_no)
    {
	for(int i = 0; i<2; ++i)
	{
	    Rect rect(dim_no);
	    m_cover.push_back(rect);
	}

	for(int i = 0; i<MAXNODES+1; ++i)
	{
	    Branch branch(dim_no);
	    m_branchBuf.push_back(branch);
	} 
    }	
...
RTREE_TEMPLATE
int RTREE_QUAL::Search(const ELEMTYPE a_min[], const ELEMTYPE a_max[], t_resultCallback a_resultCallback, void* a_context)
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
...
void RTREE_QUAL::SplitNode(Node* a_node, const Branch* a_branch, Node** a_newNode)
{
  ...
  PartitionVars localVars(NUMDIMS);
