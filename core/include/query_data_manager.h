...
enum OperationCode
{
    ...
    TAL_LOAD_SUBTAR,        /*!<DML operation that creates a Subtar and attaches Datasets to it. */
    TAL_SHOW,               /*!<DML operation that lists TARs and Subtars. */ 
    TAL_SCAN,               /*!<DDL operation to allow fully TAR retrieval as is. */
    TAL_SELECT,             /*!<DDL operation that projects dimensions and attributes from the TAR. */
    TAL_FILTER,             /*!<DDL operation that applies a bitmask aux TAR and removes TAR cells. */
    TAL_SUBSET,             /*!<DDL operation that cuts the TAR creating a TAR subset. */
    TAL_LOGICAL,            /*!<DDL operation that combines bitmasks aux TARs using lofical operations. */
    TAL_COMPARISON,         /*!<DDL operation that creates bitmasks aux TARs using comparison operations. */
    TAL_ARITHMETIC,         /*!<DDL operation that creates derived attributes by executing an arithmetic expression. */
    TAL_CROSS,              /*!<DDL operation that creates a TAR as the carterian product of two other TARS. */
    TAL_EQUIJOIN,           /*!<DDL operation that creates a cartersin product of two TARs and them filters the result.*/
    TAL_DIMJOIN,            /*!<DDL operation that creates a TAR intersecting cells with matching indexes for a set of paierd dimensions. */
    TAL_SLICE,              /*!<DDL operation that removes creates a TAR by slicing. */
    TAL_AGGREGATE,          /*!<DDL operation that calculate aggregation functions with TARS. */
...
enum QueryType
{
    DDL, /*!<DDL operations are the ones that create and/or remove the structures in the database. */
    DML  /*!<DML operations are the ones that manipulate/transform the data in the database. */
};
...
class Operation
{
    ...
    * @param param is the parameter for which the textual representation must be created.
...
public:
    ...
    * @param name is a string containing the name of the parameter to be retrieved..
...
/**A QueryPlan encompasses the operations that need to be executed in order to answer the user query */
class QueryPlan
{ 
    QueryType _type;                /*!<Query type: DML or DDL. */
    list<OperationPtr> _operations; /*!<List of operation references in the QueryPlan. */
...
public:
    ...
    * Adds a new operation in the query plan.  
    * @param operation reference to be added in the query plan.
    * @param idCounter is a integer counter reference to assign sequential TAR names.
    */
...
/**A QueryDataManager is the module responsible for keeping all query related data , such
...
class QueryDataManager : public SavimeModule
{
    ...
    * @param queryPart is a string containing a part of a query string to be concatenated. 
    ...
    * @param error is a string containing the error message to be sent to the client.
    */
    virtual void SetErrorResponseText(string error) = 0;
    
    /**
    * Gets the error text response string.
    * @return A string cotaining the current erro message to be sent to the client.
    */
    ...
    * @param paramName is the name of the parameter whose file path is to be retrieved.
    ...
    */
    ...
    /**
    * Releases all the resources used by the query plan except the created param files. 
